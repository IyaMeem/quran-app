    ...
    <div class="container py-6">
        <div class="bg-white p-4 shadow rounded">
            <div class="flex -mx-4 items-center mb-6">
                <div class="flex-1 px-4">
                    <select @change="getSpecificSurah" class="quran-input" name="" id="">
                        ...
                        <option v-for="surah in surahs" :value="surah.number">{{surah.number}} - {{ surah.name }} - {{ surah.englishName }}</option>
                    </select>
                </div>
                ...
                <div class="flex-1 px-4 text-center">
                    ...
                    <p class="font-bold text-xs">{{currentSurah.revelationType}}</p>
                </div>
            ...
            <div v-else class="text-2xl">
               <p class="flex items-center mb-6" v-if="currentSurah.hasOwnProperty('ayahs')" v-for="ayah in currentSurah.ayahs"><span class="text-xs inline-block w-5 h-5 flex items-center justify-center border rounded-full mr-4 mt-5">{{ayah.numberInSurah}}</span>  {{ayah.text}}</p>
           </div>
        </div>
    </div>
 </div>
...
export default {
  ...
  mounted () {
    ...
    axios.get('http://api.alquran.cloud/v1/surah/1')
        .then(response => {
            //console.log(response.data.data)
